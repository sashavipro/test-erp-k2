...
class OrderService:
    ...
    Including automatic calculation of total amounts and validation of items.
    """
    ...
    async def create_order(self, order_in: OrderCreate) -> OrderResponse:
        ...
        Validates the existence of the client and products, and automatically
        computes the total amount based on product prices.
        """
        ...
        # We need the order id before creating items?
        # Actually we can create Order, add items to its `items` relationship.
        ...
        for item_in in order_in.items:
            ...
            order_item = OrderItem(
                order=order, product_id=product.id, quantity=item_in.quantity
            )
            order_items.append(order_item)

        order.total_amount = total_amount
        logger.info(
            f"Successfully created order with total amount: {total_amount} "
            f"for client: {order_in.client_id}"
        )
